...
def draw_pattern(h1,h2,num_space):
    
    num_colors = 3 # change to allow user to parametrize
    wi = 5 # change to allower user to parametrize
    
    h1 = height_1(h1,wi,num_colors)
    h2 = height_2(h2,wi,num_colors)
    ...
    st = h1 + w + h2 + chunk + h1
    new_st = draw_space(st,num_space)
...
def rect_basket2D_dic(height1, height2, space_size,palette):
    ...
    for i in range(0, basket_h):
        for j in range(0, basket_w ):
            ...
            if j >= nonpattern_space and j < basket_w - nonpattern_space:
                color = col_dic[col_lst[i][j - nonpattern_space]]
            
            else: 
                color = col_dic['-']
                
            rect = Rectangle( bottom_left, weave_w, 1, fill = True, facecolor = color, edgecolor = "white", linewidth = 1)
            square_dic[rect] = color
...
def rect_basket2D_rec(height1, height2, space_size,palette,option):
    col_dic = get_color_test(palette)
    rect_param = draw_pattern(height1, height2, space_size)
    col_lst = pattern_str_to_list(rect_param)
    nonpattern_space = 10
